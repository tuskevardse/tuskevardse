...
export default function NewsPage() {
  return (
    <>
      ...
      <section className="py-18 sm:py-24" data-editor-block="news.content">
        <Container>
          <SectionHeading
          ...
          <div className="mt-12 grid gap-6">
            {newsItems.map((item, index) => (
              <Card
                ...
                className="grid gap-5 lg:grid-cols-[180px_minmax(0,1fr)_220px] lg:items-start"
                data-editor-block={`news.item.${index}`}
              >
                <div className="rounded-[1.4rem] bg-brand-sky/75 px-4 py-4">
                  <p className="text-sm font-semibold uppercase tracking-[0.16em] text-brand-blue" data-editor-text={`news.item.${index}.category`}>
                    {item.category}
                  </p>
                  <p className="mt-3 text-base font-medium text-brand-navy" data-editor-text={`news.item.${index}.date`}>
                    {item.date}
                  </p>
                </div>
                <div>
                  <h2 className="text-2xl font-semibold tracking-[-0.03em] text-ink" data-editor-text={`news.item.${index}.title`}>
                    {item.title}
                  </h2>
                  <p className="mt-4 text-base leading-7 text-muted" data-editor-text={`news.item.${index}.summary`}>
                    {item.summary}
                  </p>
                </div>
                <div className="rounded-[1.4rem] bg-white/70 px-4 py-4 ring-1 ring-brand-navy/10">
                  <p className="text-sm font-semibold uppercase tracking-[0.16em] text-brand-blue" data-editor-text={`news.item.${index}.audienceLabel`}>
                    Kinek szól?
                  </p>
                  <p className="mt-3 text-base leading-7 text-ink" data-editor-text={`news.item.${index}.audience`}>
                    {item.audience}
                  </p>
                </div>
              </Card>
          ...
          <Card className="mt-10 bg-[linear-gradient(135deg,rgba(19,34,56,0.96),rgba(18,60,115,0.95))] text-white ring-0" data-editor-block="news.ctaCard">
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-center">
              ...
              <div className="flex flex-col gap-3">
                <ButtonLink data-editor-text="news.ctaCard.action.0" href="/esemenyek" size="lg" variant="secondary">
                  Események megnyitása
                </ButtonLink>
                <ButtonLink data-editor-text="news.ctaCard.action.1" href="/kapcsolat" size="lg" variant="ghost">
                  Kapcsolatfelvétel
                </ButtonLink>
              </div>
            </div>
